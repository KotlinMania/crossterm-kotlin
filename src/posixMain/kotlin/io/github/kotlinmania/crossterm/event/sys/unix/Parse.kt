...
import io.github.kotlinmania.crossterm.event.KeyEventState
import io.github.kotlinmania.crossterm.event.KeyModifiers
import io.github.kotlinmania.crossterm.event.KeyboardEnhancementFlags
import io.github.kotlinmania.crossterm.event.MediaKeyCode
import io.github.kotlinmania.crossterm.event.ModifierKeyCode
import io.github.kotlinmania.crossterm.event.MouseButton
import io.github.kotlinmania.crossterm.event.MouseEvent
import io.github.kotlinmania.crossterm.event.MouseEventKind
 ...
 */
@HiddenFromObjC
class ParseEventException(
    message: String = "Could not parse an event.",
) : Exception(message)
...
private fun parseCsiPrimaryDeviceAttributes(buffer: ByteArray): ParseResult = ParseResult.Success(InternalEvent.PrimaryDeviceAttributes)
 ...
 */
fun parseKeyEventKind(kind: UByte): KeyEventKind =
    when (kind.toInt()) {
...
fun parseCsiBracketedPaste(buffer: ByteArray): ParseResult {
    require(
        buffer.startsWith(byteArrayOf(0x1B, '['.code.toByte(), '2'.code.toByte(), '0'.code.toByte(), '0'.code.toByte(), '~'.code.toByte())),
    ) {
