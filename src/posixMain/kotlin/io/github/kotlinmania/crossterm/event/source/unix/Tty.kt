 ...
 */
@OptIn(ExperimentalForeignApi::class)
private fun nonblockingUnixPair(): Pair<Int, Int> =
    memScoped {
...
class TtyInternalEventSource private constructor(
    ...
    companion object {
         ...
         * @return A new [TtyInternalEventSource] instance.
         */
        internal fun fromFileDescriptor(
            inputFd: Int,
            closeOnDrop: Boolean = false,
        ): TtyInternalEventSource = TtyInternalEventSource(inputFd, closeOnDrop)
    }
     ...
     */
    override fun waker(): Waker? =
        wakePipe?.waker?.let { tw ->
        ...
        in '0'.code.toByte()..'9'.code.toByte() -> {
            ...
            if (buffer.size < 4) {
                ...
                if (lastByte !in 64..126) {
                    ...
                    if (buffer.size >= 6 &&
                        buffer.sliceArray(0..5).contentEquals(
                            byteArrayOf(
                                0x1B,
                                '['.code.toByte(),
                                '2'.code.toByte(),
 ...
 */
private fun keyEvent(code: KeyCode): InternalEvent = InternalEvent.Event(Event.Key(KeyEvent.new(code)))
...
private fun ttyParseCsiPrimaryDeviceAttributes(buffer: ByteArray): TtyParseResult =
    TtyParseResult.Success(InternalEvent.PrimaryDeviceAttributes)
 ...
 */
private fun ttyParseKeyEventKind(kind: Int): KeyEventKind =
    when (kind) {
 ...
 */
private fun ttyTranslateCodepoint(codepoint: Int): KeyCode =
    when (codepoint) {
        13 -> KeyCode.Enter
        9 -> KeyCode.Tab
