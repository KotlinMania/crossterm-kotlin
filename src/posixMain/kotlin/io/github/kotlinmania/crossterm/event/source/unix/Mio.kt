...
class MioInternalEventSource private constructor(
    ...
    companion object {
         ...
         * @return A new [MioInternalEventSource] instance.
         */
        fun fromFileDescriptor(
            fd: Int,
            closeOnDrop: Boolean = false,
        ): MioInternalEventSource = MioInternalEventSource(fd, closeOnDrop)
                ...
                InternalEvent.Event(
                    Event.Key(
                        io.github.kotlinmania.crossterm.event.KeyEvent(
                            io.github.kotlinmania.crossterm.event.KeyCode
                                .Char(char),
 ...
 */
private fun mioParseEscapeSequence(
    buffer: ByteArray,
    inputAvailable: Boolean,
): MioParseResult {
    if (buffer.size == 1) {
...
private fun mioParseCsiTildeSequence(buffer: ByteArray): MioParseResult {
    ...
    val numStr =
        buffer
            .sliceArray(2 until buffer.size - 1)
            .map { it.toInt().toChar() }
            .joinToString("")
