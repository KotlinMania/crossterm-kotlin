...
import io.github.kotlinmania.crossterm.winapi.ControlKeyState
import io.github.kotlinmania.crossterm.winapi.EventFlags
import io.github.kotlinmania.crossterm.winapi.KeyEventRecord
import io.github.kotlinmania.crossterm.winapi.MouseEventRecord
 ...
 */
private fun tryEnsureCharCase(
    ch: Char,
    desiredCase: CharCase,
): Char =
    when (desiredCase) {
        CharCase.LowerCase ->
            if (ch.isUpperCase()) {
...
internal fun getCharForKey(keyEvent: KeyEventRecord): Char? {
    ...
    val desiredCase =
        if (isShiftPressed xor isCapsLockOn) {
            CharCase.UpperCase
        } else {
            CharCase.LowerCase
        }
...
internal fun parseKeyEventRecord(keyEvent: KeyEventRecord): WindowsKeyEvent? {
    ...
    val isOnlyAltModifier =
        KeyModifiers.ALT in modifiers &&
