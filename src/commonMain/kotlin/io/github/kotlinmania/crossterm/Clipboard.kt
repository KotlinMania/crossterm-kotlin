...
sealed class ClipboardType {
    ...
    companion object {
         ...
         */
        fun fromChar(c: Char): ClipboardType =
            when (c) {
                'c' -> Clipboard
                'p' -> Primary
                else -> Other(c)
 ...
 */
data class ClipboardSelection(
    val clipboards: List<ClipboardType>,
) {
    /**
    ...
    companion object {
         ...
         */
        fun fromString(s: String): ClipboardSelection = ClipboardSelection(s.map { ClipboardType.fromChar(it) })
    }
}
