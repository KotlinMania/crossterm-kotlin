...
sealed class InternalEvent {
    /** A public event. */
    data class Event(
        val event: io.github.kotlinmania.crossterm.event.Event,
    ) : InternalEvent()
    ...
    data class KeyboardEnhancementFlags(
        val flags: io.github.kotlinmania.crossterm.event.KeyboardEnhancementFlags,
    ) : InternalEvent()
...
private object EventReaderHolder {
     ...
     */
    fun <T> withReader(block: (InternalEventReader) -> T): T =
        lock.withLock {
 ...
 * @return The result of the block.
 */
internal fun <T> lockEventReader(block: (InternalEventReader) -> T): T = EventReaderHolder.withReader(block)
 ...
 * @return The result of the block, or null if the lock could not be acquired.
 */
internal fun <T> tryLockEventReaderFor(
    duration: Duration,
    block: (InternalEventReader) -> T,
): T? = EventReaderHolder.tryWithReader(duration, block)
