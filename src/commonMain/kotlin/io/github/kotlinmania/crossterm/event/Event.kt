 ...
 */
data class KeyboardEnhancementFlags(
    val bits: UByte,
) {
    companion object {
    ...
    operator fun contains(other: KeyboardEnhancementFlags): Boolean = (bits.toInt() and other.bits.toInt()) == other.bits.toInt()
}
 ...
 */
data class PushKeyboardEnhancementFlags(
    val flags: KeyboardEnhancementFlags,
) : Command {
    override fun writeAnsi(writer: Appendable) {
        writer.append(csi(">"))
...
sealed class Event {
    ...
    fun asKeyReleaseEvent(): KeyEvent? =
        when (this) {
     ...
     */
    @HiddenFromObjC
    fun asResizeEvent(): Pair<UShort, UShort>? =
        when (this) {
...
data class KeyEvent(
    ...
    companion object {
        ...
        ): KeyEvent = KeyEvent(code, modifiers, KeyEventKind.Press, KeyEventState.NONE)

        fun from(code: KeyCode): KeyEvent = KeyEvent(code, KeyModifiers.empty(), KeyEventKind.Press, KeyEventState.empty())
...
sealed class KeyCode {
    ...
    /** A modifier key */
    data class Modifier(
        val modifierKeyCode: ModifierKeyCode,
    ) : KeyCode() {
        override fun toString(): String = super.toString()
    }
...
data class KeyModifiers(
    ...
    operator fun plus(other: KeyModifiers): KeyModifiers = KeyModifiers((bits.toInt() or other.bits.toInt()).toUByte())

    operator fun contains(other: KeyModifiers): Boolean = (bits.toInt() and other.bits.toInt()) == other.bits.toInt()
